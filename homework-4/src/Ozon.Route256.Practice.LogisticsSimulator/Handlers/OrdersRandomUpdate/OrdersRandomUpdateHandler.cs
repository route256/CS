...
public class OrdersRandomUpdateHandler : IOrdersRandomUpdateHandler
{
    ...
    public OrdersRandomUpdateHandler(
    ...
    public async Task<HandlerResult> Handle(CancellationToken token)
    {
        ...
        await PublishOrders(updatedOrders, token);
    ...
    private async Task PublishOrders(
        IEnumerable<Order> orders,
        CancellationToken ct)
    {
        var produceTasks = orders
            .Select(order => _orderEventProducer
                .Produce(order, ct));
